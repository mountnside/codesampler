					...
					int       nCmdShow )
{
	...
	g_hWnd = CreateWindowEx( NULL, "MY_WINDOWS_CLASS", 
		                     "Direct3D (DX9) - Simple Vertex Shader Using CG",
...
void initShader( void )
{
	...
	/*
	// Here's a way to build the vertex declaration when you're loading a \
	// shader and you're not sure what the settings should be.
	...
	*/
	...
	assert( cgD3D9TypeToSize( cgGetParameterType(g_CGparam_constColor) ) == 4 );
...
void setShaderConstants( void )
{
	...
	// every time the shader is invoked, but that would be wasteful if it was 
