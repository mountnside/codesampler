...
D3DXVECTOR3	g_vEye(-48.0f, 11.7f, -46.0f); // Eye Position
D3DXVECTOR3	g_vLook(0.6f, -0.2f, 0.6f);   // Look Vector
                    ...
                    int       nCmdShow )
{
    ...
    g_hWnd = CreateWindowEx( NULL, "MY_WINDOWS_CLASS", 
		                     "Direct3D (DX9) - Tokamak Chain Sample Using Ball-Joints",
...
void render( void )
{
	...
	for( int i = 0; i < NUM_RED_CUBES; i++ )
	{
		if( g_redCubes[i] )
		{
			...
			D3DXMatrixScaling( &matScale, 1.2f, 0.5f, 0.5f  ); // Scale it so it matches the Tokamak object
	...
	D3DXMatrixScaling( &matScale, 50.0f, 2.0f, 50.0f ); // Scale it so it matches the Tokamak object
	matWorld = matScale * matWorld;
	g_pd3dDevice->SetTransform( D3DTS_WORLD, &matWorld );
