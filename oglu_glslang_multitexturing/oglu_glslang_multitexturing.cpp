//-----------------------------------------------------------------------------
//           Name: oglu_glslang_multitexturing.cpp
//         Author: Anudhyan Boral
//  Last Modified: 02/27/06
//    Description: This sample demonstrates how to perform multitexturing 
//                 with a fragment shader using OpenGL's new high-level shading  
//                 language GLslang.
//
//   Control Keys: F1 - Toggle usage of fragment shader.
//				   NUMPAD 1 - Display first texture.
//				   NUMPAD 2 - Display second texture
//				   NUMPAD 3 - Perform multitexturing
//
// Note: Multitexturing is done in the fragment shader. An uniform bool is used 
//       to toggle multitexturing. This bool is set in render(); The texels of 
//       two textures are multiplied together to form the final fragment color. 
//-----------------------------------------------------------------------------
...
GLuint    g_textureID2 = 1;


GLhandleARB g_programObj;
GLhandleARB g_fragmentShader;

int g_UseTex1=1;					   //Sample uses these values
int g_UseTex2=2;					   //to control the displaying
int g_UseBoth=3;					   //of the textures
					   
int g_TextureState=3;
					...
					int       nCmdShow )
{
	...
	g_hWnd = CreateWindowEx( NULL,"MY_WINDOWS_CLASS",
						    "OpenGL - Simple MultiTexturing Using GLslang",
							WS_OVERLAPPEDWINDOW,
    ...
    switch( msg )
	{
        case WM_KEYDOWN:
		{
			switch( wParam )
			{
				...
				case VK_NUMPAD1:
					g_TextureState=g_UseTex1;
					break;

				case VK_NUMPAD2:
					g_TextureState=g_UseTex2;
					break;

				case VK_NUMPAD3:
					g_TextureState=g_UseBoth;
					break;
...
void init( void )
{
	...
	AUX_RGBImageRec *pTextureImage = auxDIBImageLoad( ".\\tex1.bmp" );
	...
	AUX_RGBImageRec *pTextureImage2 = auxDIBImageLoad( ".\\tex2.bmp" );
...
void initShader( void )
{
    ...
    //
    // Create a program object and attach the compiled fragment shader.
    //
...
void render( void )
{
	...
	if( g_bUseShaders == true )
	{
		...
		texture2ID=glGetUniformLocationARB(g_programObj,"Texture2");
		// Set "Texture2" to 1. So we use glActiveTextureARB(GL_TEXTURE1_ARB)
		glUniform1iARB(texture2ID,1);
		// Get the variable ID of multitexture
		int boolMultitextureID=glGetUniformLocationARB(g_programObj,"multitexture");

		//Using Texture 1 only
		if(g_TextureState==g_UseTex1)
		{
			// Set multitexturing to false
			glUniform1iARB(boolMultitextureID,false);
			// Set first texture as both GL_TEXTURE0_ARB and GL_TEXTURE1_ARB
		...
		//Using Texture 2 only
		if(g_TextureState==g_UseTex2)
		{
			// Set multitexturing to false
			glUniform1iARB(boolMultitextureID,false);
			// Set second texture as both GL_TEXTURE0_ARB and GL_TEXTURE1_ARB
		    glActiveTextureARB(GL_TEXTURE0_ARB);
			glBindTexture(GL_TEXTURE_2D,g_textureID2);
			glActiveTextureARB(GL_TEXTURE1_ARB);
		 	glBindTexture(GL_TEXTURE_2D,g_textureID2);
		...
		//Using both the textures
		if(g_TextureState==g_UseBoth)
		{
			// Set multitexturing to true
			glUniform1iARB(boolMultitextureID,true);
			//Set first texture as GL_TEXTURE0_ARB
			//and second texture as GL_TEXTURE1_ARB
		    glActiveTextureARB(GL_TEXTURE0_ARB);
			glBindTexture(GL_TEXTURE_2D,g_textureID);
			glActiveTextureARB(GL_TEXTURE1_ARB);
		 	glBindTexture(GL_TEXTURE_2D,g_textureID2);
