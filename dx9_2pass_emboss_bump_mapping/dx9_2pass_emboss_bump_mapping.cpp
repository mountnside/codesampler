//-----------------------------------------------------------------------------
//           Name: dx9_2pass_emboss_bump_mapping.cpp
//         Author: Kevin Harris
...
LPDIRECT3DTEXTURE9      g_baseTexture   = NULL;
D3DLIGHT9               g_pLight0;
					...
					int       nCmdShow )
{
	...
	g_hWnd = CreateWindowEx( NULL, "MY_WINDOWS_CLASS", 
		                     "Direct3D (DX9) - Two-Pass Emboss Bump Mapping",
...
void loadTexture( void )
{
    ...
    //D3DXCreateTextureFromFile( g_pd3dDevice, "alpha_test.tga", &g_baseTexture );
...
{	
    ...
    // vector across all triangles that make use it, which inturn, produces  
...
{
    Vertex *pv = NULL;
    g_pVertexBuffer->Lock( 0, sizeof(g_quadVertices), (void**)&pv, 0 );
...
void renderQuadWithEmbossBumpMapping( void )
{
	...
	//
	// Texture Stage 0: (select height map)
	//

	g_pd3dDevice->SetTexture(0, g_baseTexture);
	g_pd3dDevice->SetTextureStageState(0, D3DTSS_TEXCOORDINDEX, 0);
	g_pd3dDevice->SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_SELECTARG1);
	g_pd3dDevice->SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TEXTURE | D3DTA_ALPHAREPLICATE);

	//
	// Texture Stage 1: (add height map to inverse (shifted) height map)
	//
	...
	g_pd3dDevice->SetTextureStageState(1, D3DTSS_COLORARG1, D3DTA_TEXTURE | D3DTA_ALPHAREPLICATE | D3DTA_COMPLEMENT);
