...
public:
	...
	void loadAnimationString( int nAnimeNumber, char *chString, 
		                      AnimeEndOption nOption, int nNextAnimation = NULL );
	HRESULT drawSprite( CDisplay *pDisplay, CSurface *pSurface );
	
	int     m_nID;
	int     m_nState;
	char	m_chType[MAX_SIZE];
	char	m_chName[MAX_SIZE];
	char	m_chBitmap[MAX_SIZE];
	double	m_dPosition_x;
	double	m_dPosition_y;
	double	m_dVelocity_x;
	double	m_dVelocity_y;
	bool	m_bVisible;
	bool	m_bCollide;
	bool    m_bAutoAnimate;
	bool    m_bActive;
