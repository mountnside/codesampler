					...
					int       nCmdShow )
{
	...
	g_hWnd = CreateWindowEx( NULL, "MY_WINDOWS_CLASS", 
		                     "Direct3D (DX9) - Lighting Vertex Shader Using CG",
...
void initShader(void)
{
	...
	assert(cgD3D9TypeToSize(cgGetParameterType(g_CGparam_worldViewInv)) == 16 );
	assert(cgD3D9TypeToSize(cgGetParameterType(g_CGparam_eyePosition)) == 4 );
	assert(cgD3D9TypeToSize(cgGetParameterType(g_CGparam_lightVector)) == 4 );
}
...
void setShaderConstants( void )
{
	...
	cgD3D9SetUniformMatrix( g_CGparam_worldViewInv, &worldViewInv );
