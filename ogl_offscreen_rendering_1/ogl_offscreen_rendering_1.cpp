...
{
	//-------------------------------------------------------------------------
	// Make the p-buffer's context current, so we can do an off-screen 
	// render to it...
	//-------------------------------------------------------------------------
	...
	GLubyte imgdata[PBUFFER_WIDTH*PBUFFER_HEIGHT*4];
	glReadPixels( 0, 0, g_pbuffer.nWidth, g_pbuffer.nHeight, GL_RGBA, GL_UNSIGNED_BYTE, imgdata );
	...
	//
	// Use the image data that we read from the p-buffer to create a new 
	// dynamic texture and then bind it.
	//
	...
	glTexParameteri( GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR );
	glTexParameteri( GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR );
	glTexImage2D( GL_TEXTURE_2D, 0, 4, g_pbuffer.nWidth, g_pbuffer.nHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, imgdata );

	//
	// Render the cube again, but this time will use our new dynamic texture
	// on it, which was created from the p-buffer's pixel data.
	//
	...
	// If you need to, you can uncomment this line and dump out the p-buffer's
	// content directly into the window's buffer when debugging.
	//glDrawPixels( g_pbuffer.nWidth, g_pbuffer.nHeight, GL_RGBA, GL_UNSIGNED_BYTE, imgdata );
