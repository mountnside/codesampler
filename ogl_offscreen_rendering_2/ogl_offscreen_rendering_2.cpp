...
//                 p-buffer involves sharing the window's rendering context 
//                 with the p-buffer's context so we can load the texture with 
//                 pixel data with a simple call to glCopyTexSubImage2D.
//                 
...
void init( void )
{
	...
	if( !wglShareLists( g_hRC, g_pbuffer.hRC ) )
	{
