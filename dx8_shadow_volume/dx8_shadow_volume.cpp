//-----------------------------------------------------------------------------
//           Name: dx8_shadow_volume.cpp
//         Author: Kevin Harris
//  Last Modified: 03/03/04
//    Description: This sample demonstrates how to create dynamic shadows with  
//                 Direct3D using a shadow volume.
//
...
LPDIRECT3DVERTEXBUFFER8 g_pFloorVB  = NULL;
LPDIRECT3DTEXTURE8      g_pFloorTex = NULL;
D3DMATERIAL8            g_pFloorMtrl;

LPD3DXMESH          g_pTeapotMesh      = NULL;
D3DMATERIAL8       *g_pTeapotMtrls     = NULL;
LPDIRECT3DTEXTURE8 *g_pTeapotTextures  = NULL;
...
D3DXVECTOR3	g_vEye(0.0f, 0.0f, -8.0f);  // Eye Position
							 ...
							 LPARAM lParam )
{
    ...
    switch( msg )
	{
		...
		case WM_MOUSEMOVE:
		{
			...
			if( bMousing_L )
			{
				int nYDiff = (ptCurrentMousePosit_L.y - ptLastMousePosit_L.y);
				int nXDiff = (ptCurrentMousePosit_L.x - ptLastMousePosit_L.x);
			...
			if( bMousing_R )
			{
				g_fSpinX -= (ptCurrentMousePosit_R.x - ptLastMousePosit_R.x);
				g_fSpinY -= (ptCurrentMousePosit_R.y - ptLastMousePosit_R.y);
			}
...
void initGeometryAndLighting( void )
{
    ...
    D3DXLoadMeshFromX( "teapot.x", D3DXMESH_SYSTEMMEM, g_pd3dDevice, NULL, 
		               &pD3DXMtrlBuffer, &g_dwTeapotNumMtrls, &g_pTeapotMesh );

	
    // We need to extract the material properties and texture names 
    // from the pD3DXMtrlBuffer
    D3DXMATERIAL *d3dxMaterials = (D3DXMATERIAL*)pD3DXMtrlBuffer->GetBufferPointer();
    g_pTeapotMtrls = new D3DMATERIAL8[g_dwTeapotNumMtrls];
    g_pTeapotTextures  = new LPDIRECT3DTEXTURE8[g_dwTeapotNumMtrls];
    ...
    g_pFloorVB->Lock( 0, sizeof(g_quadVertices), (BYTE**)&pVertices, 0 );
	...
	ZeroMemory( &g_pFloorMtrl, sizeof(D3DMATERIAL8) );
	...
	ZeroMemory( &g_light0, sizeof(D3DLIGHT8) );
...
void renderShadowToScene( void )
{
    ...
    g_pd3dDevice->SetVertexShader( ShadowVertex::FVF_Flags );
    g_pd3dDevice->SetStreamSource( 0, m_pBigSquareVB, sizeof(ShadowVertex) );
...
void render( void )
{
	...
	//*
	//
	// Rendering the teapot mesh first allows the teapot to self-shadow or 
	// cast a shadow on itself, but this causes graphical artifacts at the 
	// shadow's edges.
	//
    ...
    g_pd3dDevice->SetStreamSource( 0, g_pFloorVB, sizeof(FloorVertex) );
    g_pd3dDevice->SetVertexShader( FloorVertex::FVF_Flags );
	...
	//
	// Rendering the teapot mesh last prevents graphical artifacts, but 
	// then the teapot can no longer self-shadow.
	//

    /*
	g_pd3dDevice->SetTransform( D3DTS_WORLD, &m_matTeapot );

	// Enable a little ambient lighting for out teapot
    ...
    */
	...
	g_pd3dDevice->SetRenderState( D3DRS_AMBIENT, NULL );
