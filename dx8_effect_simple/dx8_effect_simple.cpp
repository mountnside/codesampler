...
void initEffect( void )
{
	...
	if( FAILED(hr) )
	{
		MessageBox(NULL,"Unable to create the effect from \"effect.fx\"",
...
void render( void )
{
	...
	// we only have one called, "TwoPassTextureBlend".
	//
    ...
    {
        g_pEffect->Pass( uPass );

		//
		// With each pass, render geometry as you normally would...
		// Our sample effect has two passes called "P0" and "P1".
		//
