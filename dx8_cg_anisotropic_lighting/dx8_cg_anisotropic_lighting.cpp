//-----------------------------------------------------------------------------
//           Name: dx8_cg_anisotropic_lighting.cpp
//         Author: Kevin Harris
...
//                 Anisotropic Lighting values by storing the precalculated 
...
LPDIRECT3DTEXTURE8      g_pAnisoLookUpTexture = NULL;
LPDIRECT3DVERTEXBUFFER8 g_pTeapotVertexBuffer = NULL;
LPDIRECT3DINDEXBUFFER8  g_pTeapotIndexBuffer  = NULL;
					...
					int       nCmdShow )
{
	...
	g_hWnd = CreateWindowEx( NULL, "MY_WINDOWS_CLASS", 
                             "Direct3D (DX8) - Anisotropic Lighting Shader Using CG",
...
void loadXFile( const char* fileName, const DWORD dwFVF )
{
	...
	D3DXLoadMeshFromX( const_cast<char*>(fileName), D3DXMESH_SYSTEMMEM, 
                       g_pd3dDevice, NULL, NULL, &g_dwNumSections, &tempMeshRaw );
...
void init( void )
{
    ...
    D3DXCreateTextureFromFile( g_pd3dDevice, "dx8_aniso.tga", &g_pAnisoLookUpTexture );
...
void initShader( void )
{
	...
	g_CGprogram = cgCreateProgramFromFile( g_CGcontext,
		                                   CG_SOURCE,
		                                   "dx8_cg_anisotropic_lighting.cg",
	                                       vertexProfile,
										   "main",
										   vertexOptions );
	...
	assert(cgD3D8TypeToSize(cgGetParameterType(g_CGparam_worldViewInv)) == 9 );
	assert(cgD3D8TypeToSize(cgGetParameterType(g_CGparam_world)) == 12 );
	assert(cgD3D8TypeToSize(cgGetParameterType(g_CGparam_lightVector)) == 3 );
	assert(cgD3D8TypeToSize(cgGetParameterType(g_CGparam_eyePosition)) == 3 );
}
...
void setShaderConstants( void )
{
	...
	// This matrix will be used to tansform the vertices from world-space to clip-space
	...
	cgD3D8SetUniformMatrix( g_CGparam_worldViewProj, &worldViewProjection );
	cgD3D8SetUniformMatrix( g_CGparam_worldViewInv, &worldViewInverse );
	cgD3D8SetUniformMatrix( g_CGparam_world, &world );
...
void render( void )
{
	...
	g_pd3dDevice->SetIndices( g_pTeapotIndexBuffer, 0);
	g_pd3dDevice->SetStreamSource(0, g_pTeapotVertexBuffer, sizeof( Vertex ) );
