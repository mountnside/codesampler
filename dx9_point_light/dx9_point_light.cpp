//-----------------------------------------------------------------------------
//           Name: dx9_point_light.cpp
//         Author: Kevin Harris
//  Last Modified: 02/17/05
//    Description: This sample demonstrates how to setup point lighting with 
...
D3DMATERIAL9            g_sphereMaterial;

bool  g_bRenderInWireFrame = false;
float g_fLightPositionY    = 2.0f;
					...
					int       nCmdShow )
{
	...
	g_hWnd = CreateWindowEx( NULL, "MY_WINDOWS_CLASS", "Direct3D (DX9) - Point Lighting",
...
void init( void )
{
    ...
    D3DXCreateSphere( g_pd3dDevice, 0.1f, 20, 20, &g_pSphereMesh, NULL );
}
...
void initLighting( void )
{
    ...
    ZeroMemory( &g_sphereMaterial, sizeof(D3DMATERIAL9) );
    g_sphereMaterial.Emissive.r = 1.0f;
    g_sphereMaterial.Emissive.g = 1.0f;
    g_sphereMaterial.Emissive.b = 0.0f;
    g_sphereMaterial.Emissive.a = 1.0f;
}
...
void render( void )
{
    ...
    g_pd3dDevice->SetMaterial( &g_sphereMaterial );

    D3DXMatrixTranslation( &matWorld, g_light0.Position.x, 
                           g_light0.Position.y, g_light0.Position.z );
    g_pd3dDevice->SetTransform( D3DTS_WORLD, &matWorld );
